...
export default function ProjectDetail() {
    const { activeProject, setActiveProject, addItem, addPayment } = useProjects();
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-white/50 text-sm italic">Project ID: {activeProject.id}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="glass-card p-6 border-l-4 border-l-sky-500">
                    ...
                    <h3 className="text-3xl font-bold">${totalItemsCost.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
                </div>
                <div className="glass-card p-6 border-l-4 border-l-emerald-500">
                    <p className="text-xs uppercase tracking-widest text-white/40 mb-1">Payments Settled</p>
                    <h3 className="text-3xl font-bold text-emerald-400">${totalPaid.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
                </div>
                <div className="glass-card p-6 border-l-4 border-l-rose-500">
                    ...
                    <h3 className="text-3xl font-bold text-rose-400">${balance.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
                </div>
            </div>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {items.map(item => (
                                    <tr key={item.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-white/60">${item.quantityPerPcs}</td>
                                        <td className="px-6 py-4 text-right font-semibold text-sky-400">${item.totalAmount.toLocaleString()}</td>
                                    </tr>
                                ))}
                                {items.length === 0 && (
                                    <tr>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {payments.map(p => (
                                    <tr key={p.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-right font-semibold text-emerald-400">${p.amount.toLocaleString()}</td>
                                    </tr>
                                ))}
                                {payments.length === 0 && (
                                    <tr>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm bg-black/60">
                    <div className="glass-card w-full max-w-md p-8 animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={(e) => {
                            ...
                            addItem(activeProject.id, section, {
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <input name="name" required className="glass-input" placeholder="e.g., Concrete Reinforcement" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col gap-1">
                                    <label className="text-sm text-white/60">Quantity</label>
                                    <input name="quantity" type="number" required className="glass-input" placeholder="0" />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <label className="text-sm text-white/60">Rate per Unit</label>
                                    <input name="rate" type="number" required className="glass-input" placeholder="0.00" />
                                </div>
                            </div>
